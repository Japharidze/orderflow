...
| <img src="docs/diagrams/icons/python.svg" width="16"> **Own Python runner** | Restartable jobs and metadata tracking. I wanted to show how it works, not hide it behind a tool |
...
| Requirement | Where |
|---|---|
| B2B database with generated data | `generators/platform.py` |
| Weblog generated by a script | `generators/weblog.py` |
| Target data warehouse | DuckDB, models in `dbt_project/models/` |
| Handle large data | Batched reads, Parquet, and set-based SQL in DuckDB. Measured on 5M weblog lines, see `docs/data_model.md` |
...
The Postgres connection string comes from `PG_URL` if you set it, and falls back
to the same values `docker-compose.yml` uses, so it runs with no `.env` at all.
...
I used Claude throughout — for working through the brief's ambiguities and the modelling decisions, and as a reviewer on code I wrote myself.
