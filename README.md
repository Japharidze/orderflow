...
| Restartable jobs | Metadata tables and idempotent steps, `src/dwh/runner.py` |
| Initial load | `make all` |
| Handle bad data | Pydantic on the way in, dbt tests after loading |
| ETL metadata | `runs` and `job_runs` tables |
...
make all      # run the whole pipeline
make reports  # print the reports
```
